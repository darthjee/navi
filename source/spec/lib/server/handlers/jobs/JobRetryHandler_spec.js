...
import { JobRetryHandler } from '../../../../../lib/server/handlers/jobs/JobRetryHandler.js';

describe("describe('JobRetryHandler'", () => {
  let res;
  ...
  it('is an instance of RequestHandlerExecutor', () => {
    expect(new JobRetryHandler({}, res)).toBeInstanceOf(RequestHandler);
  });
  ...
  describe('#handle', () => {
    describe('when the job is in the failed queue', () => {
      ...
      it('calls retryJob with the id param', () => {
        new JobRetryHandler({ params: { id: 'abc-123' } }, res).handle();
    ...
    describe('when the job does not exist', () => {
      ...
      it('throws a NotFoundError', () => {
        expect(() => new JobRetryHandler({ params: { id: 'nonexistent' } }, res).handle())
    ...
    describe('when the job is in a non-retryable state', () => {
      ...
      it('throws a ConflictError', () => {
        expect(() => new JobRetryHandler({ params: { id: 'abc-123' } }, res).handle())
          .toThrowError(ConflictError);
      });

      it('does not call retryJob', () => {
        try { new JobRetryHandler({ params: { id: 'abc-123' } }, res).handle(); } catch (_) { /* expected */ }
