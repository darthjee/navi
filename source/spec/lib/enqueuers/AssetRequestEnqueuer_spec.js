...
describe('AssetRequestEnqueuer', () => {
  ...
  describe('#enqueue', () => {
    it('calls HtmlParser.parse with the raw HTML, selector, attribute, and logContext', () => {
      spyOn(HtmlParser, 'parse').and.returnValue([]);
      new AssetRequestEnqueuer(rawHtml, assetRequest, jobRegistry, clientRegistry).enqueue(logContext);
      expect(HtmlParser.parse).toHaveBeenCalledOnceWith(rawHtml, assetRequest.selector, assetRequest.attribute, logContext);
    });
