...
import LogsHelper from '../../../src/components/elements/helpers/LogsHelper.jsx';

describe('LogsHelper', () => {
  describe('.build', () => {
    it('returns a LogsHelper instance', () => {
      const helper = LogsHelper.build([]);
      expect(helper).toBeInstanceOf(LogsHelper);
    });
  });
  ...
  describe('#render', () => {
    ...
    describe('with log entries', () => {
      ...
      beforeEach(async () => {
        const helper = LogsHelper.build(logs);
        await act(async () => { root.render(helper.render(bottomRef)); });
      });
