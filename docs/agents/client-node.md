...
Logging comes from the published `deku-sprout` npm package (`"deku-sprout": "^0.1.0"`, imported as `import { Logger } from 'deku-sprout'`) — the same `Logger`/`BaseLogger`/`ConsoleLogger` stack the engine uses. The client depends on the published package rather than a `file:../logger` reference, since it ships standalone and cannot rely on a sibling folder existing in a consumer's install. The effective level is read from the `LOG_LEVEL` env var (default `info`), overridable per-run via the `--log-level <level>` CLI flag (which takes precedence when both are given), one of `debug`/`info`/`warn`/`error`/`silent`. Under `debug`, `ConfigFileParser` logs deduped per-`$VAR`/`${VAR}` interpolation lines (name, set/unset status, and — when set — value length + a short hash, never the raw value) plus a per-file summary, and `NaviApiClient#post` logs every outbound request (method, URL, body) — the `Authorization` header/bearer token is never logged, at any level.
