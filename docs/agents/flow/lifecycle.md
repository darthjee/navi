...
When `web.autostart` is `false`, the application boots directly into `stopped` instead of `running` — see [Web Server](../web-server.md#enginestart-request-and-response) for the `PATCH /engine/start` request/response contract that resumes it (optionally naming which resources to enqueue).
