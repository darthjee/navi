...
7. **Custom headers per `emit`** — the `client` already provides base headers. It may be necessary to add specific headers per `emit` (e.g., explicit `Content-Type: application/json`). **Status: resolved in #702.** `emit` accepts an optional `headers` map. Values may carry `$VAR` / `${VAR}` references, resolved at config load time by the same whole-file `EnvStringResolver` pass that resolves client headers. The map is merged over the resolved client headers, with `emit.headers` winning on key collision (never a full override). Only static string values are supported — computed/dynamic headers (HMAC signing, per-request correlation IDs) remain out of scope.
