...
- The resource-lookup/enqueue logic was extracted into `source/lib/utils/ResourceEnqueuer.js` (not originally called out in Step 3) to keep `ApplicationInstance.js` as small as possible. Despite that, `ApplicationInstance.js` was already at 298/300 lines before this issue and now sits at ~330, tripping the project's `max-lines` ESLint rule (warning-level only, does not fail `yarn lint` or CI). Fully resolving it would mean extracting the pre-existing lifecycle methods (`pause`/`stop`/`continue`/`start`/`restart`/`shutdown`) into their own class — a much larger, unrelated refactor left out of this PR's scope; flagged here for a possible follow-up issue.
